...
couponDetailView.prototype = {
    ...
    getItemById: function(id) {
        ...
        for (var i in J2G[lang]) {
            if (J2G[lang][i].id == id) {
                result = J2G[lang][i]
                result.path = J2GFolder
                break
            }
        }

        if ($.isEmptyObject(result)) {
            for (var i in spot[lang]) {
                if (spot[lang][i].id == id) {
                    result = spot[lang][i]
                    result.path = spotFolder
                    break
                }
            }
        }
    ...
    createSlider: function(data) {
        var slider = '<ul class="p-slider js-image-slider">'
        if (data.img1 != '') {
            slider += '<li><img src="' + image_site + data.path + data.img1 + '"></li>'
        }
        if (data.img2 != '') {
            slider += '<li><img src="' + image_site + data.path + data.img2 + '"></li>'
        }
        if (data.img3 != '') {
            slider += '<li><img src="' + image_site + data.path + data.img3 + '"></li>'
        }
        if (data.img4 != '') {
            slider += '<li><img src="' + image_site + data.path + data.img4 + '"></li>'
        }
        if (data.img5 != '') {
            slider += '<li><img src="' + image_site + data.path + data.img5 + '"></li>'
        }
    ...
    createCoupon: function(data) {
        ...
        for (var i in data.coupons) {
            coupon += "<li><a class='js-modal-open' operId='spotGetCoupon' unitId='" + data.coupons[i].id + "' couponId=" + data.coupons[i].id + " data-target='modal-coupon'>" +
                ...
                "<dd>" + words[lang].titles.getCoupon + "</dd></dl>" +
                "</div></a></li>"
        }
        coupon += '</ul>'
        return coupon
    ...
    createPrintCoupon: function() {
        $('#coupons a').on('click', function() {
            ...
            $('.p-coupon__lang #condition').html('【' + result.title + '】' + '<br/>' + result.condition)
            ...
            if (lang != 'ja') {
                $('.p-coupon__lang').css('border-bottom', '2px solid #333')
            ...
            couponDetailView.showQrcode(service_site + '/webcoupon.html' + '?terminal=' + couponDetailView.terminalId + '&coupon=' + result.id + '&lang=' + lang, 'couponQrcode')
        })
    },
    createCard: function(data) {
        var card = $('<tr><th>' + words[lang].addressCaption + '</th><td>' + data.department + data.prefecture + data.city + data.town + data.street + '</td></tr>' +
    ...
    createMap: function(data) {
        ...
        this.mapUrl = 'https://www.google.com/maps/embed/v1/directions?key=AIzaSyD8-fboN_arxAoJ9vXDf-ZxeLmxrby7_Io&origin=' + this.latitude + ',' + this.longitude + '&destination=' + data.latitude + ',' + data.longitude + '&mode=walking&language=' + langParm
        //Disable google map component. var routeMap = $("<div><iframe sandbox='allow-scripts' width='450' height='300' frameborder='0' style='zoom:2' style='border:0' src='" + this.mapUrl + "'</div>")
