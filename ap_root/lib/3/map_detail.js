...
mapDetail.prototype = {
    init: function() {
        ...
        if (fileExt == '.pdf') {
            ...
            $('#pdf').append('<embed  height="1000" width="100%" src="../rf_images/map/' + data.pdf + '#toolbar=0&navpanes=0&scrollbar=1"></embed>')
        } else {
            $jq('#pdf').append('<img id="imageFullScreen" height="1000" width="100%" src="../rf_images/map/' + data.pdf + '#toolbar=0"></img>')
        ...
        var url = imgPDF_site + data.pdf + '?terminal=' + mapDetailView.terminalId        this.showQrcode(url)
    },
