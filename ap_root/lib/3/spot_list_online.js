...
spotListView.prototype = {
    init: function() {
        ...
        if (condition.genre.length > 0) {
            $('.p-btn-genre').empty().text(words[lang].genre[condition.genre])
        }
    ...
    createItem: function(condition) {
        try {
            ...
            for (var i in data) {
                info = {
                    image: '<p><img src="../rf_images/spot/' + data[i].img1 + '"></p>',
                    name: '<p class="p-iw-title">' + data[i].name + '</p>',
                    button: '<p class="p-iw-btn">' +
                        '<a href="' + this.spotDetailURL +
                        '?unitId=' + data[i].id +
                        '&lang=' + lang + '"' +
                        ' unitId=' + data[i].id + ' operId=' + this.spotDetailOper + ' onclick="Logs.writeLog(\'spotOnlinePageName\', \'spotDetailOperName\')">' + words[lang].titles.detail + '</a></p>',
